...
import { validateInstanceName, validateForm } from '../../types/validators';
...
export default function CreateInstancePage() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    engineType: 1, // MySQL por defecto
    userId: 0, // requerido por el backend
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const finalValue = name === 'engineType' || name === 'userId' ? parseInt(value, 10) : value;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await apiService.createInstance({
        ...
        userId: formData.userId || (user ? parseInt(user.id, 10) : 0),
      } as any);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2 group">
              ...
              <input
                ...
                placeholder="Ej: MySQL Produccion"
              />
            ...
            <div className="space-y-2 group">
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full px-4 py-3 bg-black border border-gray-700 rounded-lg text-white focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition-all duration-300 hover:border-gray-600"
              >
